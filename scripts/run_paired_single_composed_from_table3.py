...
DEFAULT_EXPERIMENT_ID = "paired_single_composed_table3_30c_2model_seed12_20260514"
DEFAULT_EVAL_MODELS = "Doubao-Seed-2.0-pro,gemini-3.1-pro-preview"
...
def paired_table3_run_is_complete(output_run_dir: Path, eval_models: list[str]) -> bool:
    paired_dir = output_run_dir / "paired_single_policy"
    return (
        (paired_dir / "paired_single_composed_summary.json").exists()
        and _judgments_cover_expected_responses(
            output_run_dir / "composed" / "response_judgments.jsonl",
            output_run_dir / "selected_items.jsonl",
            eval_models,
        )
        and _judgments_cover_expected_responses(
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Run single-policy vs composed-policy control from completed Table 3 COPAL model-eval runs. "
            "The runner reuses old Table2 paired_single_policy rows when selected Table3 seed items share "
            "the same composed item ids."
        )
    )
    parser.add_argument("--experiment-id", default=DEFAULT_EXPERIMENT_ID)
    parser.add_argument("--source-table3-experiment-ids", required=True)
    parser.add_argument("--runs-dir", type=Path, default=Path("runs"))
    parser.add_argument("--prompts-path", type=Path, default=DEFAULT_PROMPTS_PATH)
    parser.add_argument("--eval-models", default=DEFAULT_EVAL_MODELS)
    parser.add_argument("--judge-model", default="gemini-3-flash-preview")
    parser.add_argument("--live-max-workers", type=int, default=8)
...
def main(argv: list[str] | None = None) -> int:
    ...
    experiment_id = args.experiment_id or f"paired_single_composed_table3_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
    ...
    source_table3_experiment_dirs = [
        args.runs_dir / "experiments" / experiment_id
        for experiment_id in parse_csv(args.source_table3_experiment_ids)
    ]
    ...
    manifest = {
        ...
        "source_table3_experiment_ids": parse_csv(args.source_table3_experiment_ids),
        "models": {
            "eval_models": eval_models,
            "judge_model": args.judge_model,
        },
        "reuse": {
            "source": "source_run_dir/paired_single_policy from Table2 seed items",
            "strategy": "prefill matching single-policy response_ids before live evaluation",
        },
        "budgets": {
    ...
    write_json(experiment_dir / "paired_table3_manifest.json", manifest)
    ...
    while True:
        processed_now = process_ready_table3_runs(
            ...
            output_experiment_dir=experiment_dir,
        ...
        aggregate = aggregate_paired_table3_outputs(
            experiment_id=experiment_id,
            output_experiment_dir=experiment_dir,
            source_table3_experiment_dirs=source_table3_experiment_dirs,
            eval_models=eval_models,
...
def process_ready_table3_runs(
    *,
    ...
    for table3_run_dir in discover_ready_table3_runs(source_table3_experiment_dirs):
        ...
        table3_judgments = _filter_judgments_by_models(
            read_jsonl(table3_run_dir / "evaluation" / "response_judgments.jsonl"),
            eval_models,
        )
        if not _rows_cover_items_for_models(table3_judgments, selected_items, eval_models):
            continue

        output_run_dir = ensure_directory(output_experiment_dir / "company_runs" / table3_run_dir.name)
        ensure_directory(output_run_dir / "composed")
        write_jsonl(output_run_dir / "selected_items.jsonl", selected_items)
        write_jsonl(output_run_dir / "composed" / "response_judgments.jsonl", table3_judgments)

        if paired_table3_run_is_complete(output_run_dir, eval_models):
            continue

        manifest = read_json(table3_run_dir / "table3_company_manifest.json")
        source_run_dir = Path(str(manifest["source_run_dir"]))
        selected_company = read_json(source_run_dir / "selected_company.json")
        ...
        grounded_rows = read_jsonl(source_run_dir / "shared_grounding" / "grounded_clauses.jsonl")
        projection_items = build_single_policy_projection_items(
            benchmark_items=selected_items,
            grounded_rows=grounded_rows,
        )
        paired_dir = ensure_directory(output_run_dir / "paired_single_policy")
        write_jsonl(paired_dir / "single_policy_projection_items.jsonl", projection_items)
        prefill_summary = prefill_legacy_single_policy_rows(
            legacy_paired_dir=source_run_dir / "paired_single_policy",
            paired_dir=paired_dir,
            projection_items=projection_items,
            eval_models=eval_models,
        )
        write_json(
            output_run_dir / "paired_table3_company_manifest.json",
            {
                "table3_run_dir": str(table3_run_dir),
        ...
        run_paired_single_policy_evaluation(
            ...
            benchmark_items=selected_items,
            grounded_rows=grounded_rows,
            composed_judgments=table3_judgments,
...
def prefill_legacy_single_policy_rows(
    *,
    legacy_paired_dir: Path,
    paired_dir: Path,
    ...
    response_count = _prefill_jsonl_by_response_id(
        source_path=legacy_paired_dir / "chatbot_responses.jsonl",
        target_path=paired_dir / "chatbot_responses.jsonl",
        expected_response_ids=expected_response_ids,
    )
    judgment_count = _prefill_jsonl_by_response_id(
        source_path=legacy_paired_dir / "response_judgments.jsonl",
        target_path=paired_dir / "response_judgments.jsonl",
        expected_response_ids=expected_response_ids,
    )
    return {
        "legacy_paired_dir": str(legacy_paired_dir),
        "expected_response_count": len(expected_response_ids),
        "prefilled_response_count": response_count,
        "prefilled_judgment_count": judgment_count,
    }


def _prefill_jsonl_by_response_id(
    *,
    source_path: Path,
    target_path: Path,
    ...
    rows_by_id = {
        ...
        if str(row.get("response_id", "")) in expected_response_ids
    }
    if target_path.exists():
        for row in read_jsonl(target_path):
            response_id = str(row.get("response_id", ""))
            if response_id and response_id not in expected_response_ids:
                raise ValueError(f"Existing paired row is not part of this Table3 run: {response_id}")
            if response_id:
                rows_by_id[response_id] = row
    if rows_by_id:
        ordered_rows = [rows_by_id[response_id] for response_id in sorted(rows_by_id)]
        write_jsonl(target_path, ordered_rows)
    return len(rows_by_id)


def aggregate_paired_table3_outputs(
    *,
    experiment_id: str,
    output_experiment_dir: Path,
    source_table3_experiment_dirs: list[Path],
    ...
    ready_count = len(discover_ready_table3_runs(source_table3_experiment_dirs))
    company_runs_dir = output_experiment_dir / "company_runs"
    if company_runs_dir.exists():
        for run_dir in sorted(path for path in company_runs_dir.iterdir() if path.is_dir()):
            if not paired_table3_run_is_complete(run_dir, eval_models):
                continue
            completed_run_ids.append(run_dir.name)
            composed_judgments.extend(read_jsonl(run_dir / "composed" / "response_judgments.jsonl"))
            projection_judgments.extend(read_jsonl(run_dir / "paired_single_policy" / "response_judgments.jsonl"))
    paired_summary = (
...
def _rows_cover_items_for_models(
    ...
    return bool(expected_response_ids) and expected_response_ids.issubset(present_response_ids)
